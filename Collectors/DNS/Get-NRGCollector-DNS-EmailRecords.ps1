...
function Get-NRGCollector-DNS-EmailRecords {
    ...
    foreach ($domain in $Domains) {
        ...
        try {
            ...
            $spfMatch = $spfRecords | Where-Object { ($_.Strings -join '') -like 'v=spf1*' } | Select-Object -First 1
            if ($spfMatch) { $d.SPF = ($spfMatch.Strings -join '') }
        ...
        try {
            ...
            $dmarcMatch = $dmarcRecords | Where-Object { ($_.Strings -join '') -like 'v=DMARC1*' } | Select-Object -First 1
            if ($dmarcMatch) { $d.DMARC = ($dmarcMatch.Strings -join '') }
        } catch { $d.DMARC = $null }

        # MTA-STS
        try {
            $mtaStsRecords = @(Resolve-DnsName -Name "_mta-sts.$domain" -Type TXT -ErrorAction Stop)
            $mtaStsMatch = $mtaStsRecords | Where-Object { ($_.Strings -join '') -like 'v=STSv1*' } | Select-Object -First 1
            if ($mtaStsMatch) { $d.MTASTS.TxtRecord = ($mtaStsMatch.Strings -join '') }
        } catch { }
        ...
        try {
            ...
            $tlsRptMatch = $tlsRptRecords | Where-Object { ($_.Strings -join '') -like 'v=TLSRPTv1*' } | Select-Object -First 1
